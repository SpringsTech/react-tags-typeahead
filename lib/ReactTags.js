...
function pressUpKey (e) {
  e.preventDefault()
  this.setState({ index: this.state.index <= 0 ? this.state.index : this.state.index - 1 })
}

function pressDownKey (e) {
  e.preventDefault()
  const size = this.state.options.length - 1
  this.setState({ index: this.state.index >= size ? this.state.index : this.state.index + 1 })
}
